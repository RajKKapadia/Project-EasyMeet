...
export default async function BookEventPage({ params }: { params: Promise<{ clerkUserId: string, eventId: string }> }) {
    ...
    const event = await db.query.EventTable.findFirst({
        where(fields, operators) {
            return operators.and(
                ...
                operators.eq(fields.id, eventId)
            )
        },
    })
    ...
    if (validTimes.length === 0) {
        return (
            <NoTimeSlots event={event} calendarUser={calendarUser}></NoTimeSlots>
        )
    }
